        ...
        if ($size_limit > 0 && ($file->getInfo('size') > $size_limit)) {
            switch ($from) {
                case 'wangeditor':
                    ...
                    break;
                case 'ueditor':
                    return json_encode(['state' => '附件过大']);
                    break;
                case 'editormd':
                    return json_encode(["success" => 0, "message" => '附件过大']);
                    break;
                case 'ckeditor':
        ...
        if ($error_msg != '') {
            switch ($from) {
                case 'wangeditor':
                    ...
                    break;
                case 'ueditor':
                    return json_encode(['state' => $error_msg]);
                    break;
                case 'editormd':
                    return json_encode(["success" => 0, "message" => $error_msg]);
                    break;
                case 'ckeditor':
            ...
            if ($file_add = AttachmentModel::create($file_info)) {
                ...
                switch ($from) {
                    case 'wangeditor':
                        ...
                        break;
                    case 'ueditor':
                        return json_encode(['state' => '上传失败']);
                        break;
                    case 'editormd':
                        return json_encode(["success" => 0, "message" => '上传失败']);
                        break;
                    case 'ckeditor':
                        return ck_js($callback, '', '上传失败');
                        break;
                    default:
                        return json_encode(['code' => 0, 'class' => 'danger', 'info' => '上传失败']);
                }
            }
        }else{
            switch ($from) {
                case 'wangeditor':
                    ...
                    break;
                case 'ueditor':
                    return json_encode(['state' => $file->getError()]);
                    break;
                case 'editormd':
                    return json_encode(["success" => 0, "message" => $file->getError()]);
                    break;
                case 'ckeditor':
                    return ck_js($callback, '', $file->getError());
                    break;
                default:
                    return json_encode(['code' => 0, 'class' => 'danger', 'info' => $file->getError()]);
            }
        }
    }
...
//                break;
    ...
    private function saveScrawl()
    {
        ...
        if (false === file_put_contents($file_path, $file_content)) {
            return json_encode(['state' => '涂鸦上传出错']);
        }
